...
//    cannot resolve with bad bracket-glob). Logic mirrors lib/auth.ts exactly.
...
async function buyBook(req: NextApiRequest): Promise<BuyResult> {
    ...
    try {
        const result = await prisma.$transaction(async tx => {
            ...
            // 6. Create Order + OrderItem using nested relation writes
            //    `user: { connect: { id } }` satisfies the checked OrderCreateInput type
            //    `items: { create: [...] }` creates the OrderItem in the same call.
            ...
            return {
                status: 200,
                message: `Successfully bought ${quality} × "${book.title}" for ${cost}. Remaining balance: ${parseFloat(purchaser.balance.toFixed(2))}`,
                data: {
                    userId,
                    bookId,
